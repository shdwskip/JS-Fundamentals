...
function GetFunctionProps(functionString) {
    let splittedFunctionString = functionString.split(' '),
        returnObject = { funcName: '', funcOperation: 'none' };
    for (let i = splittedFunctionString.length - 1; i >= 0; i--) {
...
function GetFunctionValue(functionOperation, functionString) {
    const newString = functionString.split(']');
    if (functionOperation === 'none') {
        return newString[0].trim();
    }
        let splittedFunctionString = newString[0].split(','),
            result = (functionOperation === 'min') ? Number.MAX_VALUE : 0,
            count = 0,
            number;
        for (let i = 0; i < splittedFunctionString.length; i++) {
            const numberString = splittedFunctionString[i].trim();
...
const zeroTestArr = ['def func sum[5, 3, 7, 2, 6, 3]',
    ...
    'sum[func6, func4]'];
// 42
const zeroTestArr1 = ['def func sum[1, 2, 3, -6]',
    'def newList [func, 10, 1]',
    'def newFunc sum[func, 100, newList]',
    '[newFunc]'];
// 111
const testArr1 = ['def definition[-100, -100, -100]',
...
const testArr2 =
    ['def test0[1]',
        ...
        '[test299]                 '];
// 5644
const testArr3 = [' def newFunc     [      123,432    , 4]',
...
// 1119
const testArr6 = ['def negFunc     [      -123,-432 ,-6474567    , 4]',
...
const testArr7 = [
    ...
    ' [test28]                                                                                                                                               '];
// 134216704
const testArr8 = ['def hmfunc     [123, 654, 1234, 64]',
    'def hm2 sum[hmfunc]',
    'def hm22 min[hm2]',
    'max[                 1,              hm22          ]'];
// 2075
const testArr9 = ['def maxy max[100   ,600,6001,-100]',
    'def combo [maxy, maxy,maxy,maxy, 5,6]',
    'def summary sum[combo, maxy, -18000]',
    'def pe6o avg[summary,5,maxy]',
    'sum[1, pe6o]'];
// 6008
const testArr10 = ['def maxy max[100]',
    'def summary [0]',
...
const testArr11 = ['def maxy max[100, 5000, 4,2,1]',
    'def summary1 [0]',
