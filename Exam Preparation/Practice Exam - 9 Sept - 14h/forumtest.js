function solve(arr) {
	function findPrice(arr, productName) {
		const productsNumber = Number(arr[0]);

		for (let i = 1; i <= productsNumber; i++) {
			const currentProductName = arr[i].match(/[a-z]+\s/g).join('').slice(0, -1);
...
return Number(arr[i].match(/\d+.*/g));
		...
		for (let i = startIndex; i < arr.length; i++) {
			...
			for (let z = 0; z < currentArray.length; z++) {
				...
				if (currentArray[z].match(/\d/g)) {
					...
					const multiplier = (Number(currentArray[z].match(/\d./g).toString()));
					const productName = currentArray[z].match(/\D/g).join('').trim();
	...
	const indexStart = (Number(arr[0]) + 2);
