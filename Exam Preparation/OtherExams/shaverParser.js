...
function solve(params) {
    ...
    for (let line = 0; line < view.length; line += 1) {
        ...
        if (view[line].indexOf('@renderSection') > -1) {
            const currentSection = view[line].substring(view[line].indexOf('"') + 1, view[line].lastIndexOf('"'));
            ...
            const condition = view[line].substring(view[line].indexOf('(') + 1, view[line].indexOf(')'));
            for (let i = 0; i < modelProps.length; i += 1) {
            ...
            const currentProp = view[line].substring(view[line].indexOf('@') + 1, view[line].lastIndexOf('<'));
            for (let i = 0; i < modelProps.length; i += 1) {
            ...
            inState = true;
            const iterrator = view[line].substring(view[line].indexOf('var ') + 4, view[line].indexOf('in') - 1);
            const currentProp = view[line].substring(view[line].indexOf('in ') + 3, view[line].indexOf(')'));
            for (let i = 0; i < modelProps.length; i += 1) {
                if (modelProps[i][0] === currentProp) {
                    line += 1;
                    const begin = line;
                    for (let indx = 0; indx < modelProps[i][1].length; indx += 1) {
                        ...
                        while (view[line].trim() !== '}') {
                            ...
                            if (view[line].indexOf(`@${iterrator}`) > -1) {
                                ...
                                for (let i = 0; i < modelProps.length; i += 1) {
                                    if (modelProps[i][0] === currentProp) {
                                        const parseLine = view[line].replace(`@${currentProp}`, modelProps[i][1]);
